...
`Get` returns an empty array for that and for a miss alike. Writes copy the sequence rather than alias
it, so a pooled buffer is safe to return as soon as the call completes. The `net8.0` floor pins
