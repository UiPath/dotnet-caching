    ...
    where T : class
{
    ...
    public ValueTask<T?> GetItemAsync(CacheKey cacheKey, string field, CancellationToken token = default) =>
        _cache.GetItemAsync<T>(GetCacheKey(cacheKey), field, null, token);
    ...
    public ValueTask<IDictionary<string, T?>> GetAsync(CacheKey cacheKey, string[] fields, CancellationToken token = default) =>
        _cache.GetAsync<T>(GetCacheKey(cacheKey), fields, null, token);
...
[ExcludeFromCodeCoverage]
public class StructHashCache<T> : IStructHashCache<T>
    where T : struct
{
    private readonly IHashCache _cache;
    private readonly ICacheKeyStrategy _cacheKeyStrategy;

    public StructHashCache(ICacheFactory cacheFactory)
    {
        _cache = cacheFactory.CreateHashCache(entityType: typeof(T), callerType: GetType());
        _cacheKeyStrategy = new DefaultCacheKeyStrategy();
    }

    public StructHashCache(IHashCache cache, ICacheKeyStrategy? cacheKeyStrategy = null)
    {
        _cache = cache;
        _cacheKeyStrategy = cacheKeyStrategy ?? new DefaultCacheKeyStrategy();
    }
